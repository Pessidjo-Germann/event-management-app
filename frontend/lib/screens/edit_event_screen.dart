...
/// Screen for editing existing events with ticket types
/// 
/// This screen uses the unified TicketType model instead of the previous
/// TicketTypeEditData class, resolving the standardization issue.
...
class _EditEventScreenState extends State<EditEventScreen> {
  ...
  Future<void> _loadEventData() async {
    ...
    try {
      final result = await EventService.getEventWithTickets(widget.eventId);
      if (result != null && mounted) {
        final eventData = result['event'] as Map<String, dynamic>;
        final ticketTypes = result['ticketTypes'] as List<TicketType>;
        
        _titreController.text = eventData['titre'] ?? '';
        _descriptionController.text = eventData['description'] ?? '';
        _lieuController.text = eventData['lieu'] ?? '';
        _capaciteController.text = eventData['capacite']?.toString() ?? '';
        _selectedDate = eventData['date_evenement'] != null 
            ? DateTime.parse(eventData['date_evenement']) 
            : null;
        _estPublie = eventData['est_publie'] ?? false;
        _imageUrl = eventData['image'];
        
        setState(() {
          _ticketTypes = ticketTypes.map((t) => t.asExisting()).toList();
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  /// Submit the form to update event
  Future<void> _submitForm() async {
    ...
    final success = await eventProvider.updateEvent(widget.eventId, eventRequest, _ticketTypes);
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(eventProvider.error ?? 'Erreur lors de la mise à jour')),
      );
    }
  }
          ...
          children: [
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (!ticketType.isNew && ticketType.quantiteVendue > 0)
                      Container(
                        margin: const EdgeInsets.only(left: 8),
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                        decoration: BoxDecoration(
                        ...
                        child: Text(
                          '${ticketType.quantiteVendue} vendus',
            ...
            if (!ticketType.isNew && ticketType.quantiteVendue > 0)
              Padding(
                ...
                child: Text(
                  'Quantité restante: ${ticketType.quantiteRestante}',
                  style: TextStyle(
                    color: ticketType.quantiteRestante > 0 ? Colors.green : Colors.red,
